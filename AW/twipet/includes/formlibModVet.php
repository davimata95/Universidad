 ...
 */
function formulario($formId, $generaCamposFormulario, $procesaFormulario, $id=null) {
  ...
  if ( ! formularioEnviado($_POST, $formId) ) {
    echo generaFormulario($formId, $generaCamposFormulario, $id);
  } else {
    $result = $procesaFormulario($_POST);
    if ( is_array($result) ) {
      // Error al procesar el formulario, volvemos a mostrarlo
      echo generaFormulario($formId, $generaCamposFormulario, $id, $result, $_POST);
    } else {
 ...
 */
function generaFormulario($formId, $generaCamposFormulario, $id=null, $errores = array(), &$datos = array()) {
  
  $action = RUTA_VETERINARIOS."/modificarVeterinario.php?id=$id";
  $html= '';
  ...
  $html .= $generaCamposFormulario($datos, $id);
  $html .= '</form>';
  return $html;
}
