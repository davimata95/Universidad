 ...
 */
function formulario($formId, $generaCamposFormulario, $procesaFormulario, $class=null) {
  ...
  if ( ! formularioEnviado($_POST, $formId) ) {
    echo generaFormulario($formId, $generaCamposFormulario, $class);
  } else {
    $result = $procesaFormulario($_POST);
    if ( is_array($result) ) {
      // Error al procesar el formulario, volvemos a mostrarlo
      echo generaFormulario($formId, $generaCamposFormulario, $class, $result, $_POST);
    } else {
 ...
 */
function generaFormulario($formId, $generaCamposFormulario, $class = null, $errores = array(), &$datos = array()) {
  ...
  $html .= '<form method="POST" action="'.$_SERVER['PHP_SELF'].'" id="'.$formId.'"';
  if ( $class ) {
    $html .= ' class="'.$class.'"';
  }
  $html .=' >';
